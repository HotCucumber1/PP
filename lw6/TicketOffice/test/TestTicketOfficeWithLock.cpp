...
TEST_CASE("Single-threaded operations", "[TicketOfficeWithLock]")
{
	TicketOfficeWithLock office(10);
...
TEST_CASE("Negative or zero ticketsToBuy throws", "[TicketOfficeWithLockWithLock]")
{
	TicketOfficeWithLock office(5);
	REQUIRE_THROWS_AS(office.SellTickets(0), std::invalid_argument);
	REQUIRE_THROWS_AS(office.SellTickets(-1), std::invalid_argument);
}

TEST_CASE("Multi-threaded total sales are correct", "[TicketOfficeWithLock]")
{
	constexpr int totalTickets = 1000;
	constexpr int threadCount = 16;
	constexpr int attemptsPerThread = 500;

	TicketOfficeWithLock office(totalTickets);
...
TEST_CASE("Multi-threaded overdemand", "[TicketOfficeWithLock]")
{
	constexpr int total = 500;
	TicketOfficeWithLock office(total);
